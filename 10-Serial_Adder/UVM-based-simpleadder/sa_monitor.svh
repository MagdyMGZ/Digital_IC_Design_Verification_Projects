...
task run_phase (uvm_phase phase);
    ...
    forever begin
        ...
        fork
            begin  // Monitor Input
                @(sa_vif.cb_monitor iff (sa_vif.cb_monitor.en_i == 1'b1)); // OR // @(posedge sa_vif.clk iff (sa_vif.cb_monitor.en_i == 1'b1));
                sa_seq_item.a[1] <= sa_vif.cb_monitor.ina;
                sa_seq_item.b[1] <= sa_vif.cb_monitor.inb;
                @(sa_vif.cb_monitor);					   // OR // @(posedge sa_vif.clk);
            ...
            begin  // Monitor Output
                @(sa_vif.cb_monitor iff (sa_vif.cb_monitor.en_o == 1'b1)); // OR // @(posedge sa_vif.clk iff (sa_vif.cb_monitor.en_o == 1'b1));
